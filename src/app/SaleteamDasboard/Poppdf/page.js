...
export default function Poppdf() {
  ...
  const [getquots,setGetQuots] = useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("i can get the PO data",poRes.data)
  ...
  return (
    <div>
      ...
      <main className="p-4 max-w-7xl mx-auto">
        ...
        <button
          onClick={() => router.push('/SaleteamDasboard/PurchaseOrder')}
        ...
        <button
          onClick={generatePDF}
          className="mb-4 ml-8 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
        >
          Generate PDF
        </button>
...
<div className="flex justify-between items-stretch w-full h-[200px] ml-1">
  ...
  <strong>Date:</strong> {new Date(getdata?.createdAt).toLocaleDateString()}
    {/* Or dynamically: {getdata?.createdAt ? new Date(getdata.createdAt).toLocaleDateString() : "N/A"} */}
  </div>
  <div>
...
<table className="w-full text-sm border-collapse border border-2">
  ...
  <div className="w-2/3 mb-4">
    ...
    <div className="mt-2 ">
      <div className="font-bold"><strong >Delivery:</strong> {getdata?.deliveryTerms || "N/A"}</div>
      <div className="font-bold"><strong>Payment:</strong> {getdata?.paymentTerms || "N/A"}</div>
      <div className="font-bold"><strong>Warranty:</strong> {getdata?.warrantyTerms || "N/A"}</div>
    </div>
  </div>
  ...
  <div className="flex justify-between items-center mt-2">
    {["/deltas.jpg", "/Schneider.png", "/phoenix.png", "/motovario.png"].map((src, idx, arr) => (
      <div
        key={idx}
        className={`w-1/4 p-1 ${
          idx === 0 ? 'text-left' : idx === arr.length - 1 ? 'text-right' : 'text-center'
        }`}
      >
        <img
          src={src}
